...
module pht_t2_9_32_128 #(parameter INDEX_SIZE = 9, parameter TAG_SIZE = 9, parameter GHR_LEN = 32)
    (
    input reset,
    input clk,
    input [127:0] GHR,
...
//    ^ GHR_usable[63:60] ^ PC[35:32];
    ...
    wire [8:0] folded_pc;
    wire [8:0] folded_ghr9;
//    folder_7b f3(.fold({48'b0,PC}),.folded(folded_pc));
    folder_9b_128 f3(.fold({112'b0,PC}),.folded(folded_pc));
    folder_9b_128 f1(.fold({96'b0, GHR_usable}),.folded(folded_ghr9));
...
//    ^GHR_usable[51:48] ^ GHR_usable[55:52] ^ GHR_usable[59:56] ^ GHR_usable[63:60];
    wire [INDEX_SIZE - 1:0] index = folded_pc ^ folded_ghr9;
...
wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr9 ^ ({folded_ghr9[7:0],folded_ghr9[8]});    
