...
module pht_t1_7_32_128 #(parameter INDEX_SIZE = 7, parameter TAG_SIZE = 7, parameter GHR_LEN = 32)
    (
    input reset,
    input clk,
    input [127:0] GHR,
    ...
    wire [6:0] folded_pc7;
    folder_7b_128 f1(.fold({112'b0,PC}),.folded(folded_pc7));
    
    wire [6:0] folded_ghr7;
    folder_7b_128 f2(.fold({96'b0,GHR_usable}), .folded(folded_ghr7));
    
    //Manually change this shit if adjusting pht index or tag values
    wire [INDEX_SIZE - 1:0] index = {GHR_usable[GHR_LEN-1:0]} ^ folded_pc7;
