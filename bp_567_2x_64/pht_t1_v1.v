...
module pht_t1_v1 #(parameter INDEX_SIZE = 5, parameter TAG_SIZE = 5, parameter GHR_LEN = 16)
    (
    input reset,
    input clk,
    input [127:0] GHR,
    ...
    wire [INDEX_SIZE - 1:0] index = folded_ghr5 ^ folded_pc5;//PC[5:0];
    
//    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ {GHR_usable} ^ (folded_ghr4 << 1);
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr5 ^ ({folded_ghr5[4:0],folded_ghr5[4]});
