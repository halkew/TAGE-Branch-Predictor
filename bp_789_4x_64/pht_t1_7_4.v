...
module pht_t1_7_4 #(parameter INDEX_SIZE = 7, parameter TAG_SIZE = 7, parameter GHR_LEN = 4)
    (
    input reset,
    input clk,
    input [63:0] GHR,
    ...
    wire [6:0] folded_pc7;
    folder_7b f1(.fold({48'b0,PC}),.folded(folded_pc7));
    
    wire [3:0] folded_ghr4;
    folder_4b f2(.fold({60'b0,GHR_usable}), .folded(folded_ghr4));
    
    //Manually change this shit if adjusting pht index or tag values
    wire [INDEX_SIZE - 1:0] index = {GHR_usable[GHR_LEN-1:0]} ^ folded_pc7;//PC[5:0];
    
//    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ {GHR_usable} ^ (folded_ghr4 << 1);
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ {GHR_usable} ^ ({folded_ghr4[2:0],folded_ghr4[3]});
