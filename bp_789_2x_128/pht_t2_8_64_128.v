...
module pht_t2_8_64_128 #(parameter INDEX_SIZE = 8, parameter TAG_SIZE = 8, parameter GHR_LEN = 64)
    (
    input reset,
    input clk,
    input [127:0] GHR,
    ...
    wire [7:0] folded_pc;
    folder_8b_128 f3(.fold({112'b0,PC}),.folded(folded_pc));
    //Manually change this shit if adjusting pht index or tag values
    wire [INDEX_SIZE - 1:0] index = GHR_usable[7:0] ^ GHR_usable[15:8] 
    ^ GHR_usable[23:16] ^ GHR_usable[31:24] ^ GHR_usable[39:32] ^ 
    GHR_usable[47:40] ^ GHR_usable[55:48] ^ GHR_usable[63:56] ^ folded_pc;
    
    wire [7:0] folded_ghr8;
    
//    folder_6b f2(.fold({48'b0,GHR_usable}),.folded(folded_ghr6));
    folder_8b_128 f2(.fold({64'b0,GHR_usable}),.folded(folded_ghr8));
...
//    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr6 ^ (folded_ghr5 << 1);//PC[TAG_SIZE - 1:0] ^ GHR_usable[TAG_SIZE - 1: 0] ^ {GHR_usable[TAG_SIZE - 2:0],1'b0};
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr8 ^ ({folded_ghr8[6:0],folded_ghr8[7]});    
