...
module pht_t2 #(parameter INDEX_SIZE = 5, parameter TAG_SIZE = 5, parameter GHR_LEN = 16)
    (
    input reset,
    input clk,
    input [127:0] GHR,
    ...
    wire [4:0] folded_pc5;
    folder_5b f3(.fold({48'b0,PC}),.folded(folded_pc5));
    //Manually change this shit if adjusting pht index or tag values
//PC[4:0];
    
    wire [4:0] folded_ghr5;
    
    folder_5b_128 f2(.fold({112'b0,GHR_usable}),.folded(folded_ghr5));
    wire [INDEX_SIZE - 1:0] index = folded_ghr5 ^ folded_pc5;
    
    //Tried to use folding here
//    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr5 ^ (folded_ghr4 << 1);//PC[TAG_SIZE - 1:0] ^ GHR_usable[TAG_SIZE - 1: 0] ^ {GHR_usable[TAG_SIZE - 2:0],1'b0};
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr5 ^ ({folded_ghr5[3:0],folded_ghr5[4]});//PC[TAG_SIZE - 1:0] ^ GHR_usable[TAG_SIZE - 1: 0] ^ {GHR_usable[TAG_SIZE - 2:0],1'b0};
