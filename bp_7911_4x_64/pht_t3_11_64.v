...
module pht_t3_11_64 #(parameter INDEX_SIZE = 11, parameter TAG_SIZE = 11, parameter GHR_LEN = 64)
    (
    input reset,
    input clk,
    input [63:0] GHR,
...
//    ^ GHR_usable[63:60] ^ PC[35:32];
    ...
    wire [10:0] folded_pc;
    wire [10:0] folded_ghr11;
//    folder_7b f3(.fold({48'b0,PC}),.folded(folded_pc));
    folder_11b_128 f3(.fold({112'b0,PC}),.folded(folded_pc));
    folder_11b_128 f1(.fold({64'b0, GHR_usable}),.folded(folded_ghr11));
...
//    ^GHR_usable[51:48] ^ GHR_usable[55:52] ^ GHR_usable[59:56] ^ GHR_usable[63:60];
    wire [INDEX_SIZE - 1:0] index = folded_pc ^ folded_ghr11;
                    
//    wire [6:0] folded_ghr7;

    
//    folder_7b f1(.fold(GHR_usable),.folded(folded_ghr7));

    
    
//    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr7 ^ (folded_ghr6 << 1);
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr11 ^ ({folded_ghr11[9:0],folded_ghr11[10]});    
