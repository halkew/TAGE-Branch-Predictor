...
module pht_t1 #(parameter INDEX_SIZE = 4, parameter TAG_SIZE = 4, parameter GHR_LEN = 32)
    (
    input reset,
    input clk,
    input [127:0] GHR,
...
//    folder_3b f2(.fold({60'b0,GHR_usable}), .folded(folded_ghr3));
    folder_4b_128 f2(.fold({120'b0,GHR_usable}), .folded(folded_ghr4));    
    
    //Manually change this shit if adjusting pht index or tag values
    wire [INDEX_SIZE - 1:0] index = folded_ghr4 ^ folded_pc4;//PC[5:0];
    
   
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ folded_ghr4 ^ ({folded_ghr4[2:0],folded_ghr4[3]});
