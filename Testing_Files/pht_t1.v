...
module pht_t1 #(parameter INDEX_SIZE = 4, parameter TAG_SIZE = 4, parameter GHR_LEN = 4)
    (
    input reset,
    input clk,
    input [63:0] GHR,
    ...
    wire [3:0] folded_pc4;
    folder_4b f1(.fold({48'b0,PC}),.folded(folded_pc4));
    
    wire [2:0] folded_ghr3;
//    folder_3b f2(.fold({60'b0,GHR_usable}), .folded(folded_ghr3));
    folder_3b f2(.fold({61'b0,GHR_usable[GHR_LEN - 2:0]}), .folded(folded_ghr3));    
    
    //Manually change this shit if adjusting pht index or tag values
    wire [INDEX_SIZE - 1:0] index = {GHR_usable[GHR_LEN-1:0]} ^ folded_pc4;//PC[5:0];
    
   
    wire [TAG_SIZE - 1:0] tag = PC[TAG_SIZE - 1:0] ^ {GHR_usable} ^ ({folded_ghr3[1:0],folded_ghr3[2]});
